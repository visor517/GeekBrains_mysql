...
DROP TRIGGER IF EXISTS update_name_description;
DROP TRIGGER IF EXISTS insert_name_description;

DELIMITER //

CREATE TRIGGER update_name_description BEFORE UPDATE ON products
FOR EACH ROW
BEGIN
...
CREATE TRIGGER insert_name_description BEFORE INSERT ON products
FOR EACH ROW
BEGIN
	IF NEW.name IS NULL AND NEW.description IS NULL THEN
		SIGNAL SQLSTATE '45000' SET message_text = 'Update Canceled. И name, и description не могут быть Null';
...
INSERT INTO products(name, description, price, catalog_id) VALUES (NULL, NULL, 7120.00, 1);
